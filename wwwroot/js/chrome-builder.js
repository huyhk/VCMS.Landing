...
const choiceLabels={background:{surface:'Nền giao diện',brand:'Màu thương hiệu',contrast:'Nền tương phản',transparent:'Trong suốt',image:'Ảnh nền'},height:{compact:'Nhỏ gọn',standard:'Tiêu chuẩn',large:'Lớn'},container:{boxed:'Giới hạn chiều rộng',full:'Toàn màn hình'},width:{auto:'Tự động',fill:'Lấp đầy','1':'Tỷ lệ 1','2':'Tỷ lệ 2','3':'Tỷ lệ 3','4':'Tỷ lệ 4'},align:{left:'Trái',center:'Giữa',right:'Phải'}};
const rowLabels={navigation:'Hàng điều hướng',topbar:'Thanh thông tin',main:'Nội dung chính',bottom:'Hàng cuối',logo:'Hàng logo',footer:'Chân trang'};
...
const nextRowKey=region=>{const used=new Set(states[region].rows.map(x=>x.key));let index=1;while(used.has(`row-${index}`))index++;return `row-${index}`};
function render(region){
 ...
 state.rows.forEach((row,ri)=>{
  ...
  const rowTitle=document.createElement('div');rowTitle.className='chrome-builder-row-title';const rowName=document.createElement('strong');rowName.textContent=rowLabels[row.key]||`Hàng ${ri+1}`;const rowKey=document.createElement('code');rowKey.textContent=row.key;rowTitle.append(rowName,rowKey);
  head.append(rowTitle,selectField('Nền',choices.background,row.background,'background',choiceLabels.background),selectField('Chiều cao',choices.height,row.height,'height',choiceLabels.height),selectField('Khung nội dung',choices.container,row.container,'container',choiceLabels.container),checkbox('Ẩn trên mobile',row.hideOnMobile,'hideOnMobile'));
  ...
  (row.columns||=[]).forEach((column,ci)=>{
   ...
   const colHead=document.createElement('div');colHead.className='chrome-builder-column-head';colHead.append(selectField('Độ rộng cột',choices.width,column.width,'width',choiceLabels.width),selectField('Căn cột',choices.align,column.align,'align',choiceLabels.align),button('×','remove-column'));col.append(colHead);
   (column.components||=[]).forEach((component,pi)=>{
    ...
    if(component.type==='logo')item.append(selectField('Logo sử dụng',['default','light'],component.variant||'default','variant',{default:'Logo chính',light:'Logo sáng'}));
...
form.addEventListener('click',event=>{
 ...
 const addRow=event.target.closest('[data-add-row]');if(addRow){const region=addRow.dataset.addRow;if(states[region].rows.length<5)states[region].rows.push({key:nextRowKey(region),container:'boxed',background:region==='footer'?'contrast':'surface',height:'standard',hideOnMobile:false,columns:[{width:'fill',align:'left',components:[]}]});render(region);return}
